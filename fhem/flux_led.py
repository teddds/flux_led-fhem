...
class WifiLedBulb():
	def __init__(self, ipaddr, port=5577):
		...
		self.__currentcolor = (0, 0, 0) 
		self.__currentWhiteLevel = 0
	...
	def __getState(self):
		...
		self.__write(msg)
		return self.__readResponse(14)
		
	def __determinePowerState(self, power_state):
	...
	def refreshState(self):
		...
		rx = self.__getState()
		power_str = self.__determinePowerState(rx[2])
		...
		if mode == "color" or mode == "ww":
			red = rx[6]
			green = rx[7]
			blue = rx[8]
			color_str = utils.color_tuple_to_string((red, green, blue))
			mode_str = "Warm White: {}% Color: {}".format(utils.byteToPercent(ww_level), format(color_str))
				
			self.__currentcolor = (red, green, blue)
			self.__currentWhiteLevel = utils.byteToPercent(ww_level)
		elif mode == "preset":
		...
		#print "state: {}".format(self.__state_str)
	...
	def turnOn(self, on=True):
		if on == self.__is_on: 
			#print "has already state {}".format(on)
			return
	...
	def setRGBW(self, r,g,b,ww, persist=True):
		if persist:
			msg = bytearray([0x31])
		else:
			msg = bytearray([0x41])
		...
		msg.append(g)
		msg.append(b)
		msg.append(utils.percentToByte(ww))
		msg.append(0xf0)
		msg.append(0x0f)
		self.__write(msg)
		
		self.__currentWhiteLevel = ww
		self.__currentcolor = (r,g,b)
		

	def fade(self, r2,g2,b2,ww2, duration=2):
		r1,g1,b1 = self.__currentcolor
		ww1 = self.__currentWhiteLevel
		
		#print "new r {} g {} b {} ww {}".format(r2,g2,b2,ww2)
		#print "current r {} g {} b {} ww {}".format(r1,g1,b1,ww1)
		
		#set defaults
		if r2 is None : r2 = r1
		if g2 is None : g2 = g1
		if b2 is None : b2 = b1
		if ww2 is None : ww2 = ww1
		
		#print "new r {} g {} b {} ww {}".format(r2,g2,b2,ww2)
			
			
		#print "current color bulb {}".format(self.__currentcolor)
		steps = 100
		sleeptime = float(duration)/float(steps)
		#print "sleeptime each step {}".format(sleeptime)
		last_ww = ww1
		for i in range(1,steps+1):
			r = r1 - int(i * float(r1 - r2)/steps)
			g = g1 - int(i * float(g1 - g2)/steps)
			b = b1 - int(i * float(b1 - b2)/steps)
			w = ww1 - int(i * float(ww1 - ww2)/steps)
			#print "set color bulb {}".format((r,g,b,w))
			self.setRGBW(r,g,b,w, True)
			time.sleep(sleeptime);
			
		
	
	def setWarmWhite(self, level, persist=True):
		r,g,b = self.__currentcolor
		self.setRGBW(r,g,b,level,persist)
		
		
	def setRgb(self, r,g,b, persist=True):
		self.setRGBW(r,g,b,self.__currentWhiteLevel,persist)
...
def parseArgs():
	...
	mode_group.add_option("-f", "--fadeduration", dest="fadeduration", default=None,
				  help="Fading duration",
				  metavar='FADINGDURATION', type="float")
	...
	parser.usage = "usage: %prog [-sS10fcwpCiltThe] [addr1 [addr2 [addr3] ...]."
	...
	mode_count = 0
	if options.color or options.ww:  mode_count += 1
...
def main():
	...
	for info in bulb_info_list:
		...
		if options.color and options.ww:
			if options.fadeduration is not None:
				#print "fading {}".format(options.fadeduration)	
				bulb.fade(options.color[0],options.color[1],options.color[2], options.ww, options.fadeduration)
			else:
				bulb.setRGBW(options.color[0],options.color[1],options.color[2],options.ww, not options.volatile)
		
		elif options.ww is not None:
			#print "Setting warm white mode, level: {}%".format(options.ww)
			if options.fadeduration is not None:
				#print "fading {}".format(options.fadeduration)	
				bulb.fade(None,None,None, options.ww, options.fadeduration)
			else:
			...
			if options.fadeduration is not None:
				#print "fading {}".format(options.fadeduration)	
				bulb.fade(options.color[0],options.color[1],options.color[2], None, options.fadeduration)
			else:
